...
namespace ServerShot.Framework.Tests.UnitTests
{
    ...
    public class When_Running_A_Queue_Processing_Module
    {
        ...
        public async Task The_Service_Bus_Is_Called_When_Processing()
        {
            //arr
            var service = new Mock<ICloudQueue>();
            var module = new Fakes.StubProcessingModule(0, UnitTestingSettings());
    ...
    internal class Fakes
    {
        ...
        public class StubProcessingModule : QueueProcessingServerShotModule<object>
        {
            ...
            public StubProcessingModule(int wait = 10, ServerShotModuleSettings settings = null)
                : base(settings)
            {
                _wait = wait;
                _recievedLimit = 1;
        ...
        public class InitialLogsMessageModule : InitialServerShotModule<object>
        {
            public InitialLogsMessageModule()
                : base(null)
            {
            ...
            {
                base.LogMessage("test");
            }
        }

        public class InitialContinuousRunModule : InitialServerShotModule<object>
        {
            private readonly int _runFor;
            private readonly int _waitTime;

            public InitialContinuousRunModule(int runFor, int waitTime)
            {
                _runFor = runFor;
                _waitTime = waitTime;
            }
